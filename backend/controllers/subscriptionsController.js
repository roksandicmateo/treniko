...
const changePlan = async (req, res) => {
  try {
    ...
    // Get current usage to validate downgrade
    const usage = await queryWithTenant(
      `SELECT clients_count, sessions_count
    ...
    const currentUsage = usage.rows[0] || { clients_count: 0, sessions_count: 0 };

    // Check if downgrade is safe (only check if new plan has limits)
    if (plan.max_clients && currentUsage.clients_count > plan.max_clients) {
      return res.status(400).json({
        error: 'Cannot downgrade',
        message: `Cannot downgrade to ${plan.display_name}. You have ${currentUsage.clients_count} clients but this plan only allows ${plan.max_clients}. Please deactivate or remove ${currentUsage.clients_count - plan.max_clients} client(s) first.`,
        currentClients: currentUsage.clients_count,
        maxClients: plan.max_clients,
        excessClients: currentUsage.clients_count - plan.max_clients,
        cannotDowngrade: true
      });
    }

    if (plan.max_sessions_per_month && currentUsage.sessions_count > plan.max_sessions_per_month) {
      return res.status(400).json({
        error: 'Cannot downgrade',
        message: `Cannot downgrade to ${plan.display_name}. You have ${currentUsage.sessions_count} sessions this month but this plan only allows ${plan.max_sessions_per_month}. Wait until next billing period or remove some sessions.`,
        cannotDowngrade: true
      });
    }
