...
const SessionRow = ({ session, showDate = false, onSessionClick }) => (
  <div
  ...
  >
    ...
    {session.status === 'no_show' && (
      <span className="text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded-full flex-shrink-0">No-show</span>
    )}
  </div>
);
...
const PackageWarningRow = ({ cp, navigate }) => {
  ...
  return (
    <div
      ...
      className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-colors ${isUrgent ? 'bg-red-50 hover:bg-red-100' : 'hover:bg-yellow-50'}`}
    >
      <span className="text-xl flex-shrink-0">{isUrgent ? '🔴' : '🟡'}</span>
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-gray-800 truncate">{cp.first_name} {cp.last_name}</p>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => navigate('/dashboard/calendar')} className="text-xs text-blue-600 hover:text-blue-700 font-medium">
                Open Calendar →
              </button>
            </div>
            {todaySessions.length === 0 ? (
              <div className="text-center py-8">
                ...
                <button onClick={() => navigate('/dashboard/calendar')} className="mt-3 text-xs text-blue-600 hover:underline">
                  Schedule a session
                </button>
              </div>
            ) : (
              <div className="divide-y divide-gray-100">
                {todaySessions.map(s => (
                  <SessionRow key={s.id} session={s} onSessionClick={handleSessionClick} />
                ))}
              </div>
            )}
          </div>
            ...
            {upcomingSessions.length === 0 ? (
              ...
              <div className="divide-y divide-gray-100">
                {upcomingSessions.map(s => (
                  <SessionRow key={s.id} session={s} showDate onSessionClick={handleSessionClick} />
                ))}
              </div>
            )}
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => navigate('/dashboard/packages')} className="text-xs text-blue-600 hover:text-blue-700 font-medium">
                All Packages →
              </button>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => navigate('/dashboard/clients')} className="text-xs text-blue-600 hover:text-blue-700 font-medium">
                All Clients →
              </button>
            </div>
