...
describe('static SEO files', () => {
  ...
  test('every sitemap URL is genuinely indexable', () => {
    // There are now two kinds of public URL, and the sitemap may only contain
    // these two:
    //
    //   1. An SPA route in PUBLIC_ROUTES, which RouteMeta marks `index, follow`
    //      with a canonical at runtime.
    //   2. A static page generated by scripts/generate-content-pages.mjs and
    //      served off disk, which ships its own robots and canonical tags in
    //      the response body.
    //
    // Anything else in the sitemap is a URL that is either noindex or does not
    // exist — which is worse than omitting it, because it teaches Google the
    // sitemap is unreliable. The static branch is checked rather than merely
    // allowlisted, so a page cannot be listed and then quietly ship noindex.
    ...
    for (const loc of locs) {
      ...
      if (Object.prototype.hasOwnProperty.call(PUBLIC_ROUTES, path)) continue;

      const file = join(process.cwd(), 'public', path.replace(/^\//, ''), 'index.html');
      expect(existsSync(file), `${loc} is in the sitemap but no page exists for it`).toBe(true);

      const html = readFileSync(file, 'utf8');
      expect(html, `${loc} does not declare itself indexable`)
        .toMatch(/<meta name="robots" content="index, follow">/);
      expect(html, `${loc} does not canonicalise to itself`)
        .toContain(`<link rel="canonical" href="${loc}">`);
    }
  });

  test('the static content pages carry no rating or review markup either', () => {
    // The honesty rule applies to every page, not only the homepage. These are
    // the pages most likely to attract a well-meaning "add some social proof".
    const sitemap = readPublic('sitemap.xml');
    const locs = [...sitemap.matchAll(/<loc>([^<]+)<\/loc>/g)].map((m) => m[1]);
    ...
    for (const loc of locs) {
      ...
      expect(block, `${loc} has no structured data`).toBeTruthy();

      const raw = JSON.stringify(JSON.parse(block[1]));
      for (const forbidden of ['aggregateRating', 'ratingValue', 'reviewCount']) {
        expect(raw.includes(forbidden), `${loc} contains ${forbidden}`).toBe(false);
      }
    }
  });
});
