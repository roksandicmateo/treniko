...
const ConsentModal = ({ clientName, onAccept, onDecline }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 dark:bg-gray-900">
        ...
        {/* What will be stored */}
        <div className="bg-gray-50 rounded-xl p-4 mb-6 space-y-2 dark:bg-gray-800">
          ...
          <ul className="text-sm text-gray-600 space-y-1 dark:text-gray-400">
            <li className="flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span> {t('consent.bodyMetrics')}</li>
            ...
            <li className="flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span> {t('consent.injuryHistory')}</li>
            <li className="flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span> Progress photos (if uploaded)</li>
            <li className="flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span> {t('consent.sessionHistory')}</li>
          </ul>
        </div>

        {/* Legal basis */}
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-6 dark:bg-amber-950/40">
        ...
        <div className="flex gap-3">
          <button
            onClick={onDecline}
            className="flex-1 py-2.5 px-4 rounded-xl border border-gray-300 text-gray-700 text-sm font-medium hover:bg-gray-50 transition-colors dark:hover:bg-gray-800 dark:text-gray-300 dark:border-gray-700"
          >
            Cancel
          </button>
          <button
